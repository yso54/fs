...
from SEMO.Data import db, dev, devname, set_must
  ...
  for i in bots:
      ...
      try:
       if not i["bot_username"] in Done:
        ...
        try:
           await user.send_message(bot_username, "تم تنصيب البوت مطوري الغالي ◗⋮◖")
        except:
           pass
        try:
...
async def stratmaked(client, message):
  ...
  if message.chat.username in OWNER:
    kep = ReplyKeyboardMarkup([[": تفعيل جميع البوتات :"],[": استخراج جلسه :"],[": تفعيل الصانع :", ": تعطيل الصانع :"], [": تفعيل الاشتراك الإجباري :", ": تعطيل الاشتراك الإجباري :"], [": حذف بوت :", ": انشاء بوت :"], [": البوتات المصنوعه :"], [": احصائيات البوتات المصنوعه :"], [": حظر بوت :", ": حظر مستخدم :"], [": الغاء حظر بوت :", ": الغاء حظر مستخدم :"], [": اذاعه عام بجميع البوتات :"], [": توجيه عام بجميع البوتات :"], [": اذاعه للمطورين :"], [": فحص البوتات :", ": تصفيه البوتات :"],[": المكالمات النشطه :"]], resize_keyboard=True)
    await message.reply_text(f"**اهلا يعم المطور عايز اي مني ،**", reply_markup=kep)
  
  else:
    kep = ReplyKeyboardMarkup([[": حذف بوت :", ": انشاء بوت :"], [": السورس :", ": استخراج جلسه :"]], resize_keyboard=True)
...
async def alivehi(client: Client, message):
    ...
    keyboard = InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton("𝗚𝗿𝗼𝘂𝗽 🖱️", url=f"{GROUP}"),
                InlineKeyboardButton("𝗖𝗵𝗮𝗻𝗻𝗲𝗹 🖱️", url=f"{CHANNEL}"),
            ],
            [
    ...
    await message.reply_video(
        video=VIDEO,
        caption="╭───── : ◖⋮◗ : ─────╮\n么 [᥉ᥱꪔ᥆ ᥱᥣkႦᥱᖇ ™](https://t.me/Yso01)\n么 [᥉᥆υᖇᥴᥱ ᥱᥣꪀᘜ᥆᥆ꪔ ™](https://t.me/almortageltechYso01)\n么 [ᘜᖇ᥆υρ ᥱᥣꪀᘜ᥆᥆ꪔ ™](https://t.me/Yso01)\n╰──── : ◖⋮◗ : ────╯\n: ƚ𝗁ᥱ Ⴆᥱ᥉ƚ ᥉᥆υᖇᥴᥱ ᥆ꪀ ƚᥱᥣᥱᘜᖇᥲ️ꪔ :",
        reply_markup=keyboard,
    )

@app.on_message(filters.command(["صاحب السورس", "يسو", "يسو", "مطور السورس",": مطور السورس :"], "") & ~filters.private)
async def deev(client: Client, message: Message):
    if not message.chat.type == enums.ChatType.PRIVATE:
     if await joinch(message):
            return
     user = await client.get_chat(chat_id="Yso01")
...
async def cloner(app: app, message):
    ...
    tokenn = await app.ask(chat_id=user_id, text="**ارسل توكن البوت :**", timeout=200)
    token = tokenn.text
    try:
    ...
    session = await app.ask(chat_id=user_id, text="**ارسل كود الجلسة**", timeout=200)
    ...
    await bot.stop()
    if message.chat.username in OWNER:
       dev = await app.ask(message.chat.id, "**هات ايدي المطور**", timeout=200)
       if dev.text == "انا":
          dev = message.chat.id
       else:
          dev = int(dev.text)
...
async def delbot(client: app, message):
  ...
  if OFF:
      if not message.chat.username in OWNER:
         return await message.reply_text(f"**الصانع معطل حالياً تواصل مع المطور لتنصيب بوتك \n Dev : @{OWNER[0]}**")
  if message.chat.username in OWNER:
   ask = await client.ask(message.chat.id, "ارسل يوزر البوت", timeout=200)
...
async def blockk(client: app, message):
 if message.chat.username in OWNER:
  ask = await client.ask(message.chat.id, "**الان ارسل اليوزرنيم**", timeout=200)
  if ask.text == "الغاء":
